...
class inflector_Core {
	...
	public static function singular($str, $count = NULL)
	{
		...
		// Note: Fixed by Eric 2009-11-17 - floats aren't caught by this so 0 would fail
		if (is_string($count) OR is_float($count))
		{
	...
	public static function plural($str, $count = NULL)
	{
		...
		// Note: fixed by Eric 2009-11-17 - floats wouldn't get casted to int so the exact match below would fail
		if (is_string($count) OR is_float($count))
		{
			// Convert to integer when using a digit string
			$count = (int) $count;
